...
# Run: ./test/chunk.sh [--service vllm|ninfer|ollama|rag] [--thinking] [optional custom prompt]
...
_DATA="$(jq -nc \
  ...
  '({model:$model,stream:true,max_tokens:150,messages:[{role:"system",content:$system},{role:"user",content:$user}]})
   + (if $service == "ninfer"
