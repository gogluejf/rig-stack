...
cmd_models() {
    case "${1:-}" in
        --help|-h)
            ...
            echo ""
            echo "  rig models init                    install a curated model bundle"
            echo "    --minimal                        install the minimal model bundle"
            echo "    --all                            install the full model bundle"
            echo ""
            echo "  rig models install <source>        install one model"
            echo "    --file <path>                    install a specific file from the source"
            echo "    --type <type>                    backend type: hf, ollama, comfy"
            echo ""
            echo "  rig models show <source>           show one model"
            echo "    --file <path>                    show one file under the selected model"
            echo "    --type <type>                    backend type: hf, ollama, comfy"
            echo ""
            echo "  rig models remove <source>         remove one model"
            echo "    --file <path>                    remove one file under the selected model"
            echo "    --type <type>                    backend type: hf, ollama, comfy"
            echo ""
            echo "Examples:"
