...
_benchmark_help() {
    ...
    echo -e "    ${YELLOW_SOFT}--model${RESET} ${CYAN}<model_name>${RESET}             ${DIM}run on one specific model for a service, all if omitted${RESET}"
    echo -e "    ${YELLOW_SOFT}--type${RESET} ${CYAN}<completion|vision>${RESET}       ${DIM}run specific tests from catalog, all if omitted${RESET}"
    echo -e "    ${YELLOW_SOFT}--log${RESET} ${CYAN}[on|off]${RESET}                   ${DIM}benchmark JSONL logging (default: on)${RESET}"
    echo ""
